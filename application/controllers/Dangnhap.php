...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='BOOKSHOP - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->from('sale.book.shop@gmail.com', 'BOOKSHOP');
            $this->email->to($email);
            $this->email->subject('Hệ thống BOOKSHOP - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng BOOKSHOP, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
        ...
        $this->data['title']='BOOKSHOP - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_port']    = '465';
            $config['smtp_timeout'] = '7';
            $config['smtp_user']    = 'sale.book.shop@gmail.com';
            $config['smtp_pass']    = 'cqfmfmrtudhcmahw';
            $config['charset']    = 'utf-8';
            $config['newline']    = "\r\n";
            $config['wordwrap'] = TRUE;
            ...
            $this->email->subject('Hệ thống BOOKSHOP - Lấy lại mật khẩu');
        ...
        $this->data['title']='BOOKSHOP - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='BOOKSHOP - Cập nhật mật khẩu mới';
